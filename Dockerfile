...
ENV PYTHONDONTWRITEBYTECODE=1 \
    ...
    SMART_DATA_AGENT_JSON_WAREHOUSE_PATH=/app/Origin_Data/mock/semantic_datasets.json
WORKDIR /app
RUN groupadd --system app && useradd --system --gid app --home /app app
COPY pyproject.toml requirements.runtime.lock README.md ./
COPY backend ./backend
COPY configs ./configs
# Only bundle deterministic demo/seed data. Delivered CSV source files may
# contain customer data and are mounted read-only at runtime.
COPY Origin_Data/__init__.py Origin_Data/metric_dictionary_seed.json ./Origin_Data/
COPY Origin_Data/mock ./Origin_Data/mock
COPY scripts ./scripts
...
RUN mkdir -p /app/runtime /app/Origin_Data /app/Topic_Data && chown -R app:app /app
USER app
EXPOSE 8787

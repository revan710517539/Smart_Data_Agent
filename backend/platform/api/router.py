...
API_ROUTE_REGISTRY = APIRouteRegistry(
    (
        ...
        RouteSpec("POST", "/api/auth/switch-tenant", "auth_switch_tenant", "auth", "Validate a live tenant assignment and atomically rotate the stateful browser session to that tenant."),
        ...
        RouteSpec("GET", "/api/tenant-governance", "tenant_governance_get", "tenancy", "List tenant topic assignments and incoming/outgoing cross-tenant resource grants."),
        RouteSpec("POST", "/api/tenant-governance", "tenant_governance_create", "tenancy", "Assign a published tenant topic or create a bounded cross-tenant resource grant."),
        RouteSpec("DELETE", "/api/tenant-governance", "tenant_governance_delete", "tenancy", "Revoke a tenant topic assignment or cross-tenant resource grant."),
        ...
        RouteSpec("GET", "/api/interaction-events/analytics", "interaction_analytics_get", "observability", "Read a global lightweight interaction analytics snapshot for the global super administrator."),
        ...
        RouteSpec("POST", "/api/data-crawler-schedule/test", "data_crawler_schedule_test", "ingestion", "Execute the current tenant-scoped SQL once to test binding, parameters and CSV delivery without saving the schedule."),

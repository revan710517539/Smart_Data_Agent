...
const checks = [
  ...
  [rail.includes('activeTab === "comments" ? "min-h-0" : "hidden"') && rail.includes('activeTab === "analysis" ? "flex h-full min-h-0 flex-1 flex-col overflow-hidden" : "hidden"') && rail.includes('activeTab === "message-board" ? "min-h-0" : "hidden"'), "评论和 AI 标签切换不得卸载内部状态"],

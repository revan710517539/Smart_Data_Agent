...
const checks = [
  ...
  [workspacePanel.includes('data-analysis-workspace-composer="true"') && weeklyRail.includes('className="h-full min-h-0"') && globalRail.includes('className="h-full min-h-0"') && rail.includes("data-context-rail-orb-clearance") && rail.includes("h-[calc(100vh-8rem)]") && rail.includes("mr-20") && rail.includes("innerHeight - top - orbClearancePx") && globalRail.includes("pb-24 pt-4") && workspacePanel.includes('data-analysis-branch-create="true"') && workspacePanel.includes("visibleThreads.filter((thread) => thread.parent_thread_id") && !workspacePanel.includes(">新建分支</button>"), "AI 分析输入框必须贴在收短并右移后的右栏底部，避开 Agent 总管浮标，且合并选项不得列出被去重的重复线程"],

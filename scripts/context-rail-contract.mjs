...
const checks = [
  ...
  [layout.includes('type TextModelLoadStatus = "idle" | "loading" | "retrying" | "ready" | "empty" | "failed"') && layout.includes("textModelRetryDelaysMs") && layout.includes("isRetryableTextModelLoadError") && layout.includes("forceRefresh: attempt > 0"), "当前模型必须区分启动重试、真实空目录和加载失败，并在 API 启动期自动刷新权威模型目录"],
  [layout.includes("正在读取模型…") && layout.includes("模型服务启动中…") && layout.includes("未接入可用文本模型") && layout.includes("模型加载失败") && layout.includes('aria-label="重新加载文本模型"'), "模型选择器不得把加载中或接口故障误报为未配置，持久失败必须提供手动重试"],
  ...
  [dashboard.includes("if (pageData.loading && !pageData.assets.length)") && dashboard.includes("<PageDataVisualizationModules controller={pageData}") && dashboard.includes("!pageData.hasSelectedPageData") && supervision.includes("!pageData.hasSelectedPageData") && !dashboard.includes('embedded />\n      {dashboardSideRail}') && weeklyRail.includes("if (!visible) return null"), "页面主数据为空时必须优先呈现已配置页面数据，且不得单独渲染页面右侧栏"],

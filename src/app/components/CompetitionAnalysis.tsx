...
const reportChartColors = ["var(--sda-report-chart-1, #3a3a3c)", "var(--sda-report-chart-2, #8e8e93)", "var(--sda-report-chart-3, #c7c7cc)", "var(--sda-report-chart-4, #636366)"];
...
export function CompetitionAnalysis() {
  ...
  const modules = competitors.length > 0 ? [
    ...
    {
      id: "radar",
      content: <div className="flex h-full min-h-0 flex-col rounded-xl border border-[#f0f0f2] bg-white p-5"><h3 className="mb-1 shrink-0 text-[13px] text-[#1d1d1f]">{productView === "consumer" ? "消费贷" : "经营贷"}综合竞争力</h3><div className="mb-2 shrink-0 text-[10px] text-[#c7c7cc]">仅对同批真实观测做 0–100 相对归一，不代表绝对评级。</div><div className="min-h-0 flex-1">{radarData.length > 0 ? <ResponsiveContainer width="100%" height="100%"><RadarChart data={radarData}><PolarGrid stroke="var(--sda-report-chart-grid, #f0f0f2)" /><PolarAngleAxis dataKey="metric" tick={{ fontSize: 10, fill: "var(--sda-report-chart-muted, #aeaeb2)" }} /><PolarRadiusAxis domain={[0, 100]} tick={false} axisLine={false} />{competitors.slice(0, 4).map((competitor, index) => <Radar key={competitor.id} name={competitor.name} dataKey={competitor.id} stroke={reportChartColors[index % reportChartColors.length]} fill={competitor.self ? reportChartColors[index % reportChartColors.length] : "none"} fillOpacity={competitor.self ? 0.08 : 0} strokeWidth={competitor.self ? 2 : 1} strokeDasharray={competitor.self ? undefined : "4 4"} />)}<Legend wrapperStyle={{ fontSize: 10 }} /></RadarChart></ResponsiveContainer> : <ChartEmpty text="观测字段不足，无法计算相对雷达。" />}</div></div>,
    },
    {
      id: "market-share",
      content: <div className="flex h-full min-h-0 flex-col rounded-xl border border-[#f0f0f2] bg-white p-5"><h3 className="mb-4 shrink-0 text-[13px] text-[#1d1d1f]">市场份额对比</h3><div className="min-h-0 flex-1">{competitors.some((item) => item.share !== undefined) ? <ResponsiveContainer width="100%" height="100%"><BarChart data={competitors.filter((item) => item.share !== undefined).map((item) => ({ id: item.id, name: item.name, 份额: item.share, self: item.self }))} layout="vertical" margin={{ left: 10 }}><CartesianGrid strokeDasharray="3 3" stroke="var(--sda-report-chart-grid, #f5f5f5)" horizontal={false} /><XAxis type="number" tick={{ fontSize: 10, fill: "var(--sda-report-chart-muted, #c7c7cc)" }} stroke="transparent" tickLine={false} unit="%" /><YAxis type="category" dataKey="name" tick={{ fontSize: 11, fill: "var(--sda-report-chart-muted, #636366)" }} stroke="transparent" tickLine={false} width={65} /><Tooltip contentStyle={{ fontSize: 11, borderRadius: 8, border: "1px solid var(--sda-report-border, #f0f0f2)" }} /><Bar dataKey="份额" radius={[0, 4, 4, 0]} barSize={20}>{competitors.filter((item) => item.share !== undefined).map((item, index) => <Cell key={item.id} fill={reportChartColors[index % reportChartColors.length]} />)}</Bar></BarChart></ResponsiveContainer> : <ChartEmpty text="暂无市场份额观测。" />}</div></div>,
    },
    {
      id: "insights",
  ...
  return (
    <div className={PAGE_DATA_PAGE_GUTTER_CLASS} data-competition-page="true">
      <StandardAnalysisPageHeader title="竞品分析" description="消费贷/经营贷分产品竞品对标 · AI竞争策略建议" metadata={loading ? "正在读取已授权市场证据…" : competitors.length > 0 ? `真实观测 ${competitors.reduce((sum, item) => sum + item.evidenceCount, 0)} 条 · 来源 ${publishers.join("、") || "未标注"} · 最近 ${formatTime(latestObservedAt)}` : undefined} stickyNote={stickyNote} editController={pageLayout.editController} canEditLayout={isSuperAdmin} leadingActions={productSwitch} pageStyleControl={isSuperAdmin && pageLayout.mode === "edit" ? <ReportPageStyleButton styleId={pageLayout.pageStyleId} onSelect={pageLayout.applyPageStyle} /> : null} headerDataAttribute="competition" />
